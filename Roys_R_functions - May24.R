...
scrb_zumi_to_umitable = function(
  ...
  message(paste0("pools from filenames: ", paste(pools,collapse=",")))
  message(paste0("pools from metadata: ", paste(unique(meta[,pool_col]),collapse=",")))
...
size_of = function(object){
  ...
  size_mb = size / (1024^2)
  message(paste0("size is ",round(size_mb,3)," mb"))
...
pdf_combine = function(input_filenames,out_filename,open=T){
  library(pdftools)
  pdf_combine(input_filenames,output=out_filename)
  if(open==T){shell.exec(out_filename)}
}

parquet_to_df = function(parquet_file,save=F){
  p = arrow::read_parquet(parquet_file, as_tibble = TRUE)
  if (save==T){
    out_path = gsub("parquet","csv",parquet_file)
    write.csv(p,out_path,row.names=F)
  } else {
